...
func (s *bfdServer) Start(ctx context.Context, config oc.BfdConfig, bindInterface string) error {
	if s.stopped.Load() {
		return errors.New("bfd server stopped")
	}

	s.bindInterface = bindInterface
...
func (s *bfdServer) startServer() {
	...
	lc.Control = func(network, address string, sc syscall.RawConn) error {
		...
		if s.bindInterface != "" {
			s.logger.Info("binding bfd listener to interface", slog.String("interface", s.bindInterface))
			if err := netutils.SetBindToDevSockopt(sc, s.bindInterface); err != nil {
				return err
			}
		}
